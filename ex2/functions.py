...
def filter_and_align_descriptors(f1: Tuple[t_points, t_descriptors], f2: Tuple[t_points, t_descriptors],
    ...
    # distance_matrix = spatial.distance.cdist(f1[1], f2[1], metric=similarity_metric)
    #
    # # Step 2: Find matching indexes
    # src_idx = np.arange(len(f1[0]))
    # dst_idx = np.argmin(distance_matrix, axis=1)
    #
    # # Step 3: Compute significant matches
    # sorted_distances = np.sort(distance_matrix, axis=1)
    # distance_ratio = sorted_distances[:, 0] / sorted_distances[:, 1]
    # significant_matches = distance_ratio < similarity_threshold
    #
    # # Step 4: Filter and return aligned points
    # aligned_points_f1 = f1[0][significant_matches]
    # aligned_points_f2 = f2[0][dst_idx][significant_matches]
    #
    # return aligned_points_f1, aligned_points_f2
